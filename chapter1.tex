...
There are multiple preprocessed versions available on this dataset. I've used a dataset provided to me by \emph{Gábor Recski}, my former consultant, currently working at \textit{Sclable} as the head of NLP.
This preprocessed dataset was in a jsonl format, each line containing a dictionary with the article and the highlights from that article and also the set of sentences from each article, that together give the highest achievable ROUGE score.
